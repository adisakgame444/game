...
export const createOrder = async (
  ...
  try {
    ...
    if (paymentMethod === "COD") {
      const hasNoCOD = cart?.products.some(
        (item) => item.product.cod === false
      );

      if (hasNoCOD) {
        throw new Error(
          "ไม่สามารถเลือกเก็บเงินปลายทางได้ เนื่องจากมีสินค้าที่ไม่รองรับ COD"
        );
      }
    }
    ...
    const newOrder = await db.$transaction(async (prisma) => {
      ...
      for (const item of cart.products) {
        ...
        if (!product || product.stock < item.count) {
          throw new Error(`สินค้า ${product?.title} มีไม่พอ`);
        }
        ...
        await prisma.orderItem.create({
          data: {
            quantity: item.count,
            price: product.price,
            totalPirce: item.count * product.price,
  ...
  try {
    ...
    const updatedOrder = await db.order.update({
      ...
      data: {
        ...
        status:
          existingOrder.paymentMethod?.toUpperCase() === "COD" ? "COD" : "Paid",
        paymentAt: new Date(),
      },
    });
...
export const cancelOrderStatus = async (orderId: string) => {
  ...
  try {
    ...
    if (order.status === "Cancelled") {
      return { message: "คำสั่งซื้อนี้ถูกยกเลิกไปแล้ว" };
    }
